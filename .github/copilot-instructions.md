...
- **Registro de cambios y versiones:**
  - Cada cambio significativo debe documentarse en un tipo historial de vercioones en el archivo *Versionamiento.md* con fecha, descripción del cambio, autor{Cy tamayo siemrpe} y versión. este archivo tiene como primera linea la version actual del sistema, cada vez que se haga un cambio significativo se debe actualizar esta version siguiendo el formato mayor.minor.patch (ejemplo: 1.0.0, 1.1.0, 2.0.0 etc) y luego agregar un nuevo registro en el historial de versiones con la fecha del cambio, una breve descripción del cambio, el autor{Cy tamayo siemrpe} del cambio y la nueva version resultante despues del cambio
  - Para cambios menores o correcciones de errores, se debe actualizar la sección de "Cambios Menores" en el mismo archivo, indicando la fecha, descripción del cambio y autor{Cy tamayo siemrpe}.
- **Documentación:** Es obligatorio mantener una documentación clara y actualizada del código, incluyendo comentarios explicativos en funciones, clases y módulos, así como una documentación general del proyecto que describa su arquitectura, flujo de datos y cualquier decisión técnica relevante.
