...
// 1) Para qué sirve: construir extensión de tema según color primario seleccionado.
// 2) Cómo funciona: para `noir` aplica esquema especial; para otros usa paleta normal.
// 3) Qué hace: define tokens semánticos usados por PrimeVue al pintar la UI.
// 4) Cómo editarla: agrega nuevas variaciones de esquema dentro de esta función.
...
// 1) Para qué sirve: actualizar color primario o superficie desde configurador.
// 2) Cómo funciona: modifica layoutConfig, aplica tema y programa guardado de preferencias.
// 3) Qué hace: sincroniza cambios visuales en tiempo real y persistencia de usuario.
// 4) Cómo editarla: agrega nuevos tipos de color en la condición `type`.
...
// 1) Para qué sirve: aplicar en caliente cambios de tema al sistema Prime.
// 2) Cómo funciona: usa updatePreset para primario y updateSurfacePalette para superficie.
// 3) Qué hace: refleja inmediatamente ajustes visuales sin recargar la app.
// 4) Cómo editarla: extiende ramas si agregas más dimensiones de tema.
...
// 1) Para qué sirve: cambiar preset completo (Aura/Lara/Nora) manteniendo configuración activa.
// 2) Cómo funciona: recompone preset+extensión+superficie con API `$t` de Prime.
// 3) Qué hace: renueva el aspecto base de la interfaz.
// 4) Cómo editarla: integra presets nuevos en `presets` y aquí en el flujo de cambio.
...
// 1) Para qué sirve: cambiar modo de menú (estático/superpuesto) desde configurador.
// 2) Cómo funciona: delega en changeMenuMode y persiste preferencia si hay sesión.
// 3) Qué hace: adapta comportamiento lateral de navegación.
// 4) Cómo editarla: agrega lógica adicional si se suman más modos de menú.
function alCambiarModoMenu(event) {
    ...
    >
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
                ...
                <SelectButton v-model="menuMode" @change="alCambiarModoMenu" :options="menuModeOptions" :allowEmpty="false" optionLabel="label" optionValue="value" />
            </div>
        </div>
    </div>
</template>
